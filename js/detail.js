...
export function makeDetail(mount, glossary, ui, casesById) {
  ...
  function open(c, lang) {
    ...
    // click anywhere outside the card (the page behind) closes it. Uses
    // pointerdown so that clicking another node/case still opens that one
    // (this fires, closing the current card, then the click opens the new).
    ...
    if (c.execution) {
      ...
      body.appendChild(h);
      const ebody = document.createElement('div'); ebody.className = 'exec-detail';
      const subBlock = (en, pl, val) => {
        if (!val || !t(val, lang)) return;
        const sh = document.createElement('p'); sh.className = 'exec-sub'; sh.textContent = lang === 'pl' ? pl : en;
        const sp = document.createElement('p'); sp.className = 'exec-sub-body'; sp.appendChild(lz(t(val, lang)));
        ebody.append(sh, sp);
      };
      subBlock('General measures required', 'Wymagane środki generalne', c.execution.generalMeasures);
      subBlock('Done so far', 'Co zrobiono', c.execution.doneSoFar);
      const sup = c.execution.supervision;
      if (sup && sup.latestAction) {
        const sh = document.createElement('p'); sh.className = 'exec-sub'; sh.textContent = lang === 'pl' ? 'Nadzór' : 'Supervision';
        const sp = document.createElement('p'); sp.className = 'exec-sub-body';
        sp.append(`${sup.body} (${sup.latestAction.date}): `, lz(t(sup.latestAction.text, lang)));
        ebody.append(sh, sp);
      }
      body.appendChild(ebody);
    }
