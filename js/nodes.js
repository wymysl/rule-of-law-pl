...
export function createNodes(cases, stage, onSelect) {
  return cases.map((c, i) => {
    ...
    // In the timeline the persistent case name is interactive too: clicking it
    // opens the case (hovering it activates the node via :hover, see CSS).
    n.querySelector('.dot-title').addEventListener('click', () => onSelect(c, n));
    stage.appendChild(n);
    return n;
  });
}
