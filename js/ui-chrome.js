...
export function renderHero(mount, primer, lang) {
  ...
  inner.append(
    el('div', 'kicker', lang === 'pl' ? 'Kryzys praworządności w Polsce' : 'The Rule of Law Crisis in Poland'),
...
export function renderPrimer(mount, primer, lang) {
  ...
  [primer.whatIsRoL, primer.howCourts].forEach(s => {
    ...
    if (s.quote) {
      const bq = el('blockquote', 'primer-quote', s.quote.text);
      bq.appendChild(el('cite', 'primer-cite', t(s.quote.cite, lang)));
      sec.appendChild(bq);
    }
...
export function renderDisclaimer(footer, primer, lang) {
  footer.innerHTML = '';
  if (primer.notExhaustive) footer.appendChild(el('p', 'disclaimer-note', t(primer.notExhaustive, lang)));
