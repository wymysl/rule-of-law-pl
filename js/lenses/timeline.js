...
export function render(ctx) {
  ...
  Object.entries(ticks).forEach(([y, el]) => {
    el.classList.add('on');
    el.style.left = (axisX + 18) + 'px'; el.style.top = scaleY(+y) + 'px';
    el.style.transform = 'translateY(-50%)';
  });

  // event landmarks down the LEFT (diamond on the axis, label to its left),
  // de-collided vertically so long labels (especially in Polish, which wraps to
  // more lines) never overlap the next landmark.
  const EV_TOPOFF = 9, EV_PAD = 12;
  let evLastBottom = -1e9, evMaxBottom = 0;
  events.map((e, i) => ({ e, i, y: scaleY(yearFraction(e.date)) }))
    .sort((a, b) => a.y - b.y)
    .forEach(({ e, i }) => {
      const el = eventEls[i];
      const etxt = el.querySelector('.etxt');
      etxt.textContent = t(e.label, lang);
      el.classList.add('on');
      el.style.left = axisX + 'px';
      el.style.transform = 'none';
      let y = scaleY(yearFraction(e.date));
      const h = etxt.offsetHeight || 18;
      if (y - EV_TOPOFF < evLastBottom + EV_PAD) y = evLastBottom + EV_PAD + EV_TOPOFF;
      el.style.top = y + 'px';
      evLastBottom = (y - EV_TOPOFF) + h;
      if (evLastBottom > evMaxBottom) evMaxBottom = evLastBottom;
    });
  ...
  stage.style.height = Math.round(Math.max(maxY, evMaxBottom) + BOT) + 'px';
}
